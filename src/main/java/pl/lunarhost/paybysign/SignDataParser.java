...
/**
  * Parsers {@link PayBySign} from {@link Sign}s or directly from its lines.
  */
public class SignDataParser {
    public Optional<PayBySign> parse(Sign sign) throws ParseException {
        Objects.requireNonNull(sign, "sign");
        return this.parse(sign, sign.getLines());
    }

    public Optional<PayBySign> parse(Sign sign, String[] lines) throws ParseException {
        Objects.requireNonNull(sign, "sign");
        Objects.requireNonNull(lines, "lines");
        Preconditions.checkArgument(lines.length == 4, "4 lines expected, " + lines.length + " given");

         // identifier
        if (!ChatColor.stripColor(lines[0]).equals(PayBySign.NAMESPACE)) {
        ...
        return Optional.of(new PayBySign(sign, playerName, price, delay));
    }
